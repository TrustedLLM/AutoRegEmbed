...
from src.pretrain.arguments import (
    ModelArguments, 
    DataArguments, 
    CompressTrainingArguments as TrainingArguments
)

from src.pretrain.data import (
    PreTrainDataset, 
    DataCollatorForDynamicPadding,
)

from src.pretrain.trainer import (
    PreTrainer
)
...
def main():
    ...
    model = CompressWithDiffTokenModel(
    ...
    train_dataset = PreTrainDataset(path=data_args.train_data_path,
        args=data_args, 
        model=model
    )
