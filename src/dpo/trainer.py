...
class DPOTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        # loss2 = F.sigmoid(self.dpo_beta*torch.abs((policy_chosen_logps-inputs['pos_batch']['dpo_score'].view(len(policy_chosen_logps),-1)[:,:1])-(policy_chosen_logps_self-inputs['pos_batch']['dpo_score'].view(len(policy_chosen_logps),-1)[:,1:2])))
        loss2 = F.sigmoid(self.dpo_beta*torch.abs(policy_chosen_logps-policy_chosen_logps_self))

        loss = -torch.log(torch.exp(-loss2/0.1) / (torch.exp(-loss2/0.1)+torch.exp(-loss/0.1)))
        # loss = -torch.log(torch.exp(-loss2/0.1) / torch.exp(-loss/0.1))
        #F.sigmoid(
        # if 
